...
const getAllOrderByShop = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (data_user) {
            ...
            const data_count = yield order_model_1.OrderModel.getCountOrderByShop({ code_shop: code_shop, type: type });
            yield order_model_1.OrderModel.getAllOrderByShop({ code_shop: code_shop, type: type }, (err, result) => {
                if (err) {
                    res.json({
                        error: err,
                    });
